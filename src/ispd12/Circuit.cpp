...
void Circuit::runBaselineFlow() {
	...
	cout << "- Enable lambda-delay sensitivities: " << optEnableLambdaDelaySensitivities << "\n";
	cout << "- Enable slack filtering: " << optEnableSlackFiltering << "\n";
	cout << "- Enable gamma: " << optEnableGamma << "\n";
	cout << "- Enable load violation filtering: " << optEnableLoadViolationFiltering << "\n";
	
	cout << "- UseTennakoon: " << optUseTennakoon << "\n";

	cout << "- Run initial sizing: " << optRunInitialSizing << "\n";	
	cout << "- Run initial sizing for load and slew violation removal: " << optRunInitialSizingForLoadAndSlewViolationRemoval << "\n";
	cout << "- Run power recovery: " << optRunPowerRecovery << "\n";
	cout << "- Run timing recovery: " << optRunTimingRecovery << "\n";
	
    initialSolution(false); //set initial solution (min leakage cells ou random cells)
    updateTiming();
	...
	std::cout << "updateTiming() runtime: " << stopwatchUpdateTiming.getElapsedTime() << " s\n";
} // end method
...
void Circuit::calcLoadViol() {
	...
	for (int i = 0; i < icells.size(); ++i) {
		...
		(tmpCell->actualLoad - tmpCell->actualInstType->pins[0].maxCapacitance) : 0.0;
...
void Circuit::calcSlewViol() {
	...
	for ( int i = timingOffsetToNetLevel.front(); i < timingOffsetToNetLevel.back(); i++ ) {
		...
		timingViolationSlew += computeSlewViolation(netstate.slew[RISE], net.fanout);
		timingViolationSlew += computeSlewViolation(netstate.slew[FALL], net.fanout);
	} // end for
...
void Circuit::updateCellType(Vcell* cell, int typeIndex) {
	...
	loadViol -= (cell->actualLoad > cell->actualInstType->pins[0].maxCapacitance) ? (cell->actualLoad - cell->actualInstType->pins[0].maxCapacitance) : 0.0;
	...
	//cout << "Celula escolhida " << cell->instType << " name " << cell->instName << endl;
} // end method
...
bool Circuit::updateCellTypeLagrangeRelaxationSensitivities(Vcell *cell, const double gamma, const double alpha) {
	...
	for (int i = 0; i < numCandidateCells; i++) {
		...
		const double sizingEffectOnLocalNegativeSlack = computeSizingEffectOnLocalNegativeSlack(n);
		if (optEnableSlackFiltering && (sizingEffectOnLocalNegativeSlack < originalSizingEffectOnLocalNegativeSlack * (optEnableGamma? gamma : 1) ))
			continue;
		
        const double costLeakage = optIgnoreLeakagePower? 0 : cell->getLeakagePower();
...
bool Circuit::updateCellTypeLagrangeRelaxationSensitivitiesTestingChanges(Vcell *cell, const double gamma, const double alpha) {
	...
	for (int i = 0; i < numCandidateCells; i++) {
		...
		// Do not accept load violations.
		if (loadViol > originalLoadViolation || ( timingNumPathsWithNegativeSlack > 2*originalPathViolation && worstSlack < 0 && ratio > 3 ))
			continue;
...
bool Circuit::upsize(Vcell * cell, const bool allowLoadViolation) {
	...
	LibCellGroupCharacterization &option = timingLibraryCharacterization[cell->footprintIndex];
...
void Circuit::updateLoads(Vcell * changedCell) {
	...
	for (int a = 0; a < changedCell->previousCells.size(); ++a) {
		...
		loadViol -= (tmpCell->actualLoad > tmpCell->actualInstType->pins[0].maxCapacitance) ? (tmpCell->actualLoad - tmpCell->actualInstType->pins[0].maxCapacitance) : 0.0;
		updateCellLoad(tmpCell);
		loadViol += (tmpCell->actualLoad > tmpCell->actualInstType->pins[0].maxCapacitance) ? (tmpCell->actualLoad - tmpCell->actualInstType->pins[0].maxCapacitance) : 0.0;
	}
...
void Circuit::initialSolution(bool random) {
	...
	if (enableReport_LogicalEffortDiscretization)
		reportFile_LogicalEffortDiscretization.open("report-logical-effort-discretization.dat");
	
	if (enableReport_LogicalEffortCharacterization)
		reportFile_LogicalEffortCharacterization.open("report-logical-effort-chaterization.dat");
	...
	if (pfile = fopen(filename.c_str(), "w")) {
		...
		for (int i = 0; i < icells.size(); ++i) {
			...
			fprintf(pfile, "%s %s size=%d vth=%d fanout=%d depth=%d paths=%d\n", 
					...
					cellChar.size,
					cellChar.vth,
					(int) tmpCell->nextCells.size(),
					tmpCell->logicalDepth,
...
void Circuit::readSDC() {
	...
	do {
		...
		if (valid) {
			...
			// [BUG FIX] After ISVLSI 2013 (PhD Thesis) - does not affect the ISVLSI results
			tmpCell->footprintIndex = orgCells.findFootprintIndex(tmpCell->actualInstType->footprint);
			tmpCell->actualInstTypeIndex = orgCells.oCells[tmpCell->footprintIndex].findCellTypeIndex(driverSize);
	...
	for (int i = 0; i < numOrgCells; i++) {
		...
		for (int k = 0; k < numCellSizes; k++) {
			...
			if (cellinfo.timingArcs.size() > 0) {
				const double outRiseDelay = Circuit::lookup(cellinfo.timingArcs[0].riseDelay, capSum, slew);
				const double outFallDelay = Circuit::lookup(cellinfo.timingArcs[0].fallDelay, capSum, slew);
...
void Circuit::stepperFoXLogicalEffort(Vcell *cell) {
	...
	const double riseSlack = -(netstate.arrivalTime[RISE] - timingRequiredTime[cell->sinkNetIndex][RISE]);
	const double fallSlack = -(netstate.arrivalTime[FALL] - timingRequiredTime[cell->sinkNetIndex][FALL]);
...
void Circuit::stepperSlewTarget(Vcell *cell) {
    ...
    //		}
    //
    //	double Circuit::lookupDelay( const LibParserTimingInfo &timingInfo, const EdgeType edgeType, const double inputSlew, const double loadCapacitance ) {
    ...
    //	computeArcTiming
    //	// Output Slew
    //	outSlew[RISE] = Circuit::lookup(timingInfo.riseTransition, xOutputLoad, yInputFallSlew );
    //	outSlew[FALL] = Circuit::lookup(timingInfo.fallTransition, xOutputLoad, yInputRiseSlew );
    //
...
void Circuit::stepperSetGlobalCriticality(Vcell* cell) {
	...
	cell->globalCriticalityOutputPinFall = timingRequiredTime[n].getFall() - netstate.arrivalTime.getFall();
	cell->globalCriticalityOutputPinRise = timingRequiredTime[n].getRise() - netstate.arrivalTime.getRise();
...
void Circuit::stepperSetCellSlewTarget(Vcell* cell) {
	...
	for (i = 0; i < numCandidateCells; i++) {
		...
		for (k = 0; k < cellInst->timingArcs.size(); ++k) {
            ...
            // Rise and Fall Delay
			outRiseDelay = Circuit::lookup(timingInfo.riseDelay, xOutputLoad, yInputFallSlew);
			outFallDelay = Circuit::lookup(timingInfo.fallDelay, xOutputLoad, yInputRiseSlew);
            
            // Output Rise and Fall Slew
			outRiseSlew = Circuit::lookup(timingInfo.riseTransition, xOutputLoad, yInputRiseSlew);
			outFallSlew = Circuit::lookup(timingInfo.fallTransition, xOutputLoad, yInputFallSlew);
...
void Circuit::computeCellDepths() {
	...
	for (int i = 0; i < icells.size(); i++) {
		if (icells[i]->logicalDepth == -1)
			cerr << "[BUG] @ computeCellDepths(): untouched cell " << icells[i]->instName 
					<< " (" << icells[i]->instType << ").\n";
	} // end for
...
void Circuit::computeWeightedNumberOfPathsPassingTrhuNets() {
	...
	for (int i = 0; i < numNets; i++) {
		...
		if (k0 == k1 || (net.driver && net.driver->logicalDepth == 0)) {
			const float wdepth = net.driver?
					net.driver->reverseLogicalDepth / float(maxReverseLogicalDepth) : 0;
			net.wipaths = wdepth;
		} // end if
	} // end for
	...
	for (int i = numNets - 1; i >= 0; i--) {
		...
		if (k0 == k1) {
			const float wdepth = net.driver?
					net.driver->logicalDepth / float(maxLogicalDepth) : 0;
			net.wopaths = wdepth;
		} // end if
	} // end for
	...
	if (true) {
		...
		for (int i = 0; i < numNets; i++) {
			...
			file << std::setw(6) << net.depth << " "
					<< std::setw(12) << net.wipaths << " "
					<< std::setw(12) << net.wopaths << " "
					<< std::setw(12) << (net.wipaths*net.wopaths) << " "
					<< std::setw(12) << (net.centrality) << " "
					<< std::setw(6) << timingNetName[i] << "\n";
		} // end method
...
void Circuit::computeLogicalEffort_ReferenceSlew() {
	...
	const LibCellGroupCharacterization &option = timingLibraryCharacterization[FOOTPRINT_INV]; 
	const LibParserCellInfo &info = orgCells.oCells[FOOTPRINT_INV].cells[option.mappingVthSizeToCellTypeIndex[0][0]];
	...
	for (int edge = 0; edge < 2; edge++) {
		...
		if (report) {
			cout << "Computing reference slew for " << (edge==RISE?"RISE":"FALL") << " transition...\n";
		...
		const LibParserLUT &lut = edge == RISE? 
			arc.riseTransition : arc.fallTransition;
		...
		for (int i = 0; i < maxIterations; i++) {
			...
			if (approximatelyEqual(prevSlew, currSlew))
				break;
			prevSlew = currSlew;
		} // end for
...
//	cout << "[WARNING] Forcing reference slew to the input driver slew...\n"; 
...
void Circuit::computeLogicalEffort_LinearLeastSquares(
...
void Circuit::computeLogicalEffort_Report(ostream &out) {
	for (int footprint = 0; footprint < orgCells.oCells.size(); footprint++) {
		const LibCellGroupCharacterization &groupChar = timingLibraryCharacterization[footprint];
		...
		for (int type = 0; type < groupChar.cellCharacterization.size(); type++) {
			...
			const LibCellCharacterization &cellChar = groupChar.cellCharacterization[type];
			...
			for (int k = 0; k < cellChar.arcs.size(); k++) {
				...
				for (int oedge = 0; oedge < 2; oedge++) {
					...
					const LibParserLUT &lut = (oedge == FALL)? larc.fallDelay : larc.riseDelay;
...
void Circuit::computeLogicalEffort() {
	...
	if (enableReport_LogicalEffortCharacterization) {
		reportFile_LogicalEffortCharacterization.open("lib-characterization-le.report");
	} // end if
	
	// Compute reference slew.
	computeLogicalEffort_ReferenceSlew();
	if (enableReport_LogicalEffortCharacterization)
		reportFile_LogicalEffortCharacterization << "Reference Slew: " << referenceSlew << "\n";
	...
	for (int footprint = 0; footprint < orgCells.oCells.size(); footprint++) {
		LibCellGroupCharacterization &groupChar = timingLibraryCharacterization[footprint];
		
		groupChar.arcs.resize(3); // HARD CODED: number of vth groups
		for (int vth = 0; vth < 3; vth++) {
			groupChar.arcs[vth].resize(orgCells.oCells[footprint].cells[0].timingArcs.size());
		...
		for (int type = 0; type < orgCells.oCells[footprint].cells.size(); type++) {
			...
			if (enableReport_LogicalEffortCharacterization) {
				reportFile_LogicalEffortCharacterization << "Cell: " << lcell.name << " "
						<< "vth=" << cellChar.vth << " "
						<< "size=" << cellChar.size << "\n";
			} // end if
			...
			for (int k = 0; k < lcell.timingArcs.size(); k++) {
				...
				const double Cin = lcell.pins[from].capacitance > 0?
					lcell.pins[from].capacitance : defaultCin;
				...
				for (int edge = 0; edge < 2; edge++) {
					const LibParserLUT &lut = (edge == FALL)? larc.fallDelay : larc.riseDelay;
					
					if (lut.loadIndices.size() > 0) {
						computeLogicalEffort_TimingArc(lut, 
								Cin, referenceSlew[edge], gains,
								arcLe.g[edge], arcLe.p[edge], arcLe.residuum[edge]);
				...
				if (enableReport_LogicalEffortCharacterization) {
					reportFile_LogicalEffortCharacterization << "\t" << larc.fromPin << "->" << larc.toPin << ": "
							<< "Cin=" << Cin << " "
							<< "g=" << arcLe.g << " "
							<< "p=" << arcLe.p << " "
		...
		if (enableReport_LogicalEffortCharacterization) {
			reportFile_LogicalEffortCharacterization << "Group: " << orgCells.oCells[footprint].footprint << "\n";
		} // end if				
...
//		fflush(pipe_gp);		
		...
		if (report && (iteration == 0 || iteration == 24 || iteration == 49 || iteration == 99)) {
			for (int deltaVth = 0; deltaVth < reportNumDeltaVths; deltaVth++) {
			...
			for (int i = 0; i < icells.size(); i++) {
				...
				const LibCellCharacterization &char0 = 
					getLibCellCharacterization(icells[i]->footprintIndex, reportSolution[i]);
				const LibCellCharacterization &char1 = 
					getLibCellCharacterization(icells[i]->footprintIndex, icells[i]->actualInstTypeIndex);
...
void Circuit::sizingLagrangeRelaxationTestingChanges(const double resetLambdas) {
	const int numArcs = timingArcs.size();
	const int numNets = timingNets.size();
	const int numCells = depthSortedCells.size();
...
void Circuit::sizingLambdaGreedyStepSize() {
	...
	UB = myRound(getTimingViolation(),2) == 0? totalLeakage : +numeric_limits<double>::max();
	LB = 0;
	...
	for (int k = offsetCombinational; k < numCells; k++) {
		...
		const int numCandidateCells = orgCells.oCells[cell->footprintIndex].cells.size();
		for (int i = 0; i < numCandidateCells; i++) {
			const double leakage = orgCells.oCells[cell->footprintIndex].cells[i].leakagePower;
	...
	for (int iteration = 0; iteration < 100; iteration++) {
		...
		cerr << setw(12) << LB << setw(12) << L << setw(12) << UB << setw(12) << stepSize;
		printTimingDigest();
		
		
		
		L = computeLagrange();
		if ( myRound(getTimingViolation(),2) == 0 )
			UB = min(UB, totalLeakage);
...
void Circuit::sizingDecreaseVthOfHighSpanCells() {
	for ( int iteration = 0; iteration < 10; iteration++ ) {
		...
		//cout <<setw(12) << timingNetSpan[n] << setw(12) << (originalInnerSlack - bestCost) << "\n";
		printTimingDigest();
	} // end for
} // end method
...
void Circuit::timingRecoveryPathCounter() {
	...
	if ( nearZero(timingTotalNegativeSlack ) ) 
		return;
	...
	do {
		...
		for ( int n = timingNumDummyNets; n < numNets; n++ ) {
			...
			if ( vth + 1 < option.mappingVthSizeToCellTypeIndex.size() ) {
				...
				int size0 = -1;
				int size1 = -1;
				for ( int s = 0; s < option.mappingVthSizeToCellTypeIndex[vth+1].size(); s++ ) {
					LibParserCellInfo &info = orgCells.oCells[cell->footprintIndex].cells[option.mappingVthSizeToCellTypeIndex[vth+1][s]];
...
double Circuit::lookup(const LibParserLUT &lut, const double x, const double y) {
...
double Circuit::lookupOutputSlew(const LibParserTimingInfo &timingInfo, const EdgeType edgeType, const double inputSlew, const double loadCapacitance) {
	const LibParserLUT &lut = edgeType == FALL ? timingInfo.fallTransition : timingInfo.riseTransition;
	const double x = loadCapacitance;
	const double y = inputSlew;
	...
	EdgeArray<double> &outSlew) 
{
	...
	// Delay
	outDelay[RISE] = Circuit::lookup(timingInfo.riseDelay, xOutputLoad, yInputFallSlew);
	outDelay[FALL] = Circuit::lookup(timingInfo.fallDelay, xOutputLoad, yInputRiseSlew);
...
void Circuit::buildTimingStructure() {
	...
	for (int i = 0; i < dummyNets - 1; i++) {
		...
		netstate.arrivalTime[RISE] -= Circuit::lookup(timingInfo.riseDelay, 0, netstate.slew[RISE]);
		netstate.arrivalTime[FALL] -= Circuit::lookup(timingInfo.fallDelay, 0, netstate.slew[FALL]);
	} // end for
...
void Circuit::updateTiming(Vcell * cell) {
	...
	while (!queue.empty()) {
		...
		if (!nearZero(deltaSlew[FALL]) || !nearZero(deltaSlew[RISE]) || !nearZero(deltaArrivalTime[FALL]) || !nearZero(deltaArrivalTime[RISE])) {
			// Put all sink nets in the queue.
			const int k0 = timingSinkNetPointers[n];
			const int k1 = timingSinkNetPointers[n + 1];
			for (int k = k0; k < k1; k++) {
...
void Circuit::updateTiming_Net(const int i, const int threadId) {
	...
	for (int k = k0; k < k1; k++) {
		...
		computeArcTiming(timingInfo, arcstate.islew, netstate.load, arcstate.delay, arcstate.oslew);
		...
		for (int outputEdge = 0; outputEdge < 2; outputEdge++) {
            ...
            //set worst output arrival time
			const double arrivalTime = arcstate.arrivalTime[inputEdge] + arcstate.delay[outputEdge];
			if (arrivalTime > maxArrivalTime[outputEdge]) {
				maxArrivalTime[outputEdge] = arrivalTime;
				maxArrivalTimeArc[outputEdge] = k;
			} // end if
...
void Circuit::updateTiming_WorstArrivalTime() {
	...
	for (int i = 0; i < numTails; i++) {
        ...
        for (int edge = 0; edge < 2; edge++) {
			...
			const double slack = timingTailNetMultiplicities[i] * (T - netstate.arrivalTime[edge]);
...
void Circuit::updateLambdasByFlachNew() {
	...
	for (int i = 0; i < numArcs; i++) {
		...
		const EdgeArray<double> q = arc.sink == -1? EdgeArray<double>(T,T) : timingRequiredTime[arc.sink];
		const EdgeArray<double> a = arcstate.arrivalTime.getReversed() + arcstate.delay;
	...
	if ( getWorstSlack() >= 0 ) {
		for (int i = numNets - 1; i >= timingNumDummyNets; i--) {
			if ( getNetSlack(i).getMin() < 0 )
				cerr << "[BUG] @ updateRequiredTime(): Worst Slack = " << getWorstSlack() << " but Net Slack = " << getNetSlack(i).getMin() << "\n";
		} // end for
	} // end if
#endif
...
void Circuit::updateTiming_Debug() {
	...
	for (int i = timingNumDummyNets; i < timingNets.size(); i++) {
		...
		EdgeArray<double> arrivalTime(
                                      sdcInfos.clk_period - cell->actualRiseSlack,
		...
		for (int edge = 0; edge < 2; edge++) {
			if (!nearlyEqual(arrivalTime[edge], net.arrivalTime[edge])) {
				...
				cerr << "\tCell: " << cell->vectorIndex << "\t" << cell->instName << "\t" << cell->instType << "\n";
	...
	for (int i = numNets - 1; i >= timingNumDummyNets; --i) {
		...
		for (int k = k0; k < k1; k++) {
			...
			for (int edge = 0; edge < 2; edge++) {
				...
				lambda[edge] = arcstate.lambda[edge] + pk * (slack[edge] - (netstate.arrivalTime[reverseEdge] - timingRequiredTime[i][reverseEdge]));
	...
	for (int i = numNets - 1; i >= timingNumDummyNets; --i) {
		...
		for (int k = k0; k < k1; k++) {
			...
			for (int edge = 0; edge < 2; edge++) {
                ...
                //Reimann: update slack at arcs using required timing in the output net (sink) and arrival time in the driver net
                slack[edge] = getTimingNetState(arc.driver).arrivalTime[edge] + arcstate.delay[reverseEdge] - requiredTimeAtSink[reverseEdge];
                const int mu_v = (netstate.arrivalTime[reverseEdge] - timingRequiredTime[i][reverseEdge]);
		...
		if (timingSinkNetPointers[i] == timingSinkNetPointers[i + 1]) {
			...
			for (int k = k0; k < k1; k++) {
				...
				for (int edge = 0; edge < 2; edge++) {
                    ...
                    lambda[edge] = arcstate.lambda[edge] + pk * ((netstate.arrivalTime[reverseEdge] - timingRequiredTime[i][reverseEdge]) /*/ max(totalLambda[edge],1.0e-12)*/);
...
void Circuit::readTimingLR() {
    ...
    do {
		...
		valid = tp.read_pin_timing(cellInst, pin, riseSlack, fallSlack, riseTransition, fallTransition);
        
        if (valid) {
            //	std::cout << " Timing for pin: " << cellInst << "/" << pin << ": " << riseSlack << " " << fallSlack << " " << riseTransition << " " << fallTransition << endl ;
    ...
    do {
		...
		valid = tp.read_port_timing(port, riseSlack, fallSlack, riseTransition, fallTransition);
        
        if (valid) {
            //s	std::cout << " Timing for port: " << port << ": " << riseSlack << " " << fallSlack << " " << riseTransition << " " << fallTransition << endl ;
        }
...
void Circuit::callPT() {
	...
	TimerInterface::Status s = TimerInterface::runTimingAnalysisBlocking(sizes, rootDir, benchName, pollingTime);
...
void Circuit::sizingDescreaseVthOfBottleneckCells() {
	...
	for (int i = 0; i < lvthThreshold; i++) {
		...
		const TimingNet &net = timingNets[n];
		Vcell * driver = net.driver;
		if (!driver || driver->dontTouch)
			continue;
		
		const TimingNetState &netstate = getTimingNetState(i);

		const double riseSlack = -(netstate.arrivalTime[RISE] - timingRequiredTime[n][RISE]);
		const double fallSlack = -(netstate.arrivalTime[FALL] - timingRequiredTime[n][FALL]);
...
double Circuit::computeLogicalEffortDelay(
		const int k, 
		const double G, 
		const double B, 
		const double P, 
...
void Circuit::logicalEffort_Reset_Cin(const int k) {
	...
	if (arc.cell) {
		...
		if (k < timingOffsetToExtraSequentialArcs) {
			if (arc.cell->actualInstType->isSequential || arc.cell->instName == "inputDriver") {
				for (int edge = 0; edge < 2; edge++) {
...
void Circuit::logicalEffort_UpdateNetLoad(const int n) {
	...
	netstate.logicalEffortLoad = net.driver?
			net.driver->wireLoad + net.driver->portLoad : 0;
	...
	for (int j = j0; j < j1; j++) {
		...
		const TimingArcState &arcstate = getTimingArcState(k);
		if (arc.cell)
			netstate.logicalEffortLoad += arcstate.logicalEffortCin;
	} // end for
} // end method
...
void Circuit::logicalEffort_Propagate() {
	...
	for (int n = timingNumDummyNets; n < numNets; n++) {
		...
		if (k0 == k1) {
			...
			for (int k = k0; k < k1; k++) {
				...
				for (int iedge = 0; iedge < 2; iedge++) {
					...
					const LogicalEffort &arcLogicalEffort = (arc.cell->dontTouch)?
							getLibArcLogicalEffortForCellSpecific(arc) :
							getLibArcLogicalEffortForCellGroup(arc);					
					
					const double H = pathFrom.pathCin > 0?
							netstate.logicalEffortLoad / pathFrom.pathCin : defaultH;

					LogicalEffortPath pathTo;
					pathTo.pathLogicalEffort = pathFrom.pathLogicalEffort * arcLogicalEffort.g[oedge];
					pathTo.pathBranching = pathFrom.pathDepth > 0? pathFrom.pathBranching : 1;
					pathTo.pathParasiticDelay = pathFrom.pathParasiticDelay + arcLogicalEffort.p[oedge];
		...
		if (enableReport_LogicalEffortPropagation) {
			reportFile_LogicalEffortPropagation << "net " << (timingNetName[n]==""?"<no name>":timingNetName[n]) << " ";
			reportFile_LogicalEffortPropagation << (net.driver? net.driver->instType : "<null>") << " ";
			for (int edge = 0; edge < 2; edge++) {
				const string edgename = (edge == FALL)? "f" : "r"; 

				reportFile_LogicalEffortPropagation << "k" << edgename << "=" << netstate.logicalEffortPath[edge].pathDepth << " ";
				reportFile_LogicalEffortPropagation << "G" << edgename << "=" << netstate.logicalEffortPath[edge].pathLogicalEffort << " ";
				reportFile_LogicalEffortPropagation << "B" << edgename << "=" << netstate.logicalEffortPath[edge].pathBranching << " ";
				reportFile_LogicalEffortPropagation << "P" << edgename << "=" << netstate.logicalEffortPath[edge].pathParasiticDelay << " ";
				reportFile_LogicalEffortPropagation << "Cin" << edgename << "=" << netstate.logicalEffortPath[edge].pathCin << " ";
			} // end for
...
void Circuit::logicalEffort_Sizing() {
	...
	for (int i = numNets - 1; i >= timingNumDummyNets; i--) {
		...
		if (report) {
			...
			file << "\tpaths " << (net.ipaths*net.opaths) << " (" << net.ipaths << "*" << net.opaths << ")\n";
		} // end if
...
//					<< "d=" << delay << "\n";
		...
		for (int edge = 0; edge < 2; edge++) {
			...
			const TimingArc &arc = timingArcs[k];
			const LibCellCharacterization &cellChar =  getLibCellCharacterization(net.driver);			
...
void Circuit::logicalEffort_Report_Delays() {
	...
	for (int k = 0; k < numArcs; k++) {
		...
		EdgeArray<double> g(numeric_limits<double>::quiet_NaN(), numeric_limits<double>::quiet_NaN());
		EdgeArray<double> p(numeric_limits<double>::quiet_NaN(), numeric_limits<double>::quiet_NaN());
		if (arc.cell) {
			const LibCellCharacterization &cellChar = getLibCellCharacterization(arc.cell);
			if (arc.lut!=-1) {
				const LibArcCharacterization &arcChar = cellChar.arcs[arc.lut];
		...
		file << "pin " << (arc.cell? arc.cell->instName : "<null>" ) << ":" << getLibPinName(arc) << " ";
...
void Circuit::logicalEffort_Report_ArrivaTimes() {
	...
	for (int k = 0; k < numArcs; k++) {
		...
		file << "diff=" << (arcstate.logicalEffortArrivalTime - arcstate.arrivalTime) << "\n";
    } // end for
} // end method
...
void Circuit::logicalEffort_UpdateTiming_Wns() {
	...
	logicalEffortWorstArrivalTime.set(-numeric_limits<double>::max(), -numeric_limits<double>::max());
	...
	for (int i = 0; i < numTails; i++) {
		...
		const TimingNetState &netstate = getTimingNetState(netindex);
        for (int edge = 0; edge < 2; edge++) {
            if (netstate.logicalEffortArrivalTime[edge] > logicalEffortWorstArrivalTime[edge]) {
                logicalEffortWorstArrivalTime[edge] = netstate.logicalEffortArrivalTime[edge];
...
void Circuit::logicalEffort_UpdateTiming_Net(const int i) {
	EdgeArray<double> maxArrivalTime(
			-numeric_limits<double>::max(), 
			-numeric_limits<double>::max());
...
void Circuit::logicalEffort_Discretize_NetDriver(const int n) {
	...
	if (!net.driver) {
		...
		const int k =
				netstate.logicalEffortArrivalTime[RISE] > 
		...
		for (int size = 0; size < groupChar.mappingVthSizeToCellTypeIndex[vth].size(); size++) {
			const int type = groupChar.mappingVthSizeToCellTypeIndex[vth][size];
			const double cin = getLibPinCapacitance(arc.cell->footprintIndex, type, arc.pin);
			
			const double error = std::abs(cin - expectedCin);
			const double loadViolation = 
					getLoadViolationForCellType(arc.cell, type);
		...
		if (enableReport_LogicalEffortDiscretization) {
			reportFile_LogicalEffortDiscretization << "Resizing cell " << arc.cell->instName 
					<< " from " << cellChar.size 
					<< " to " << bestSize 
					<< " expectedCin=" << expectedCin
					<< " actualCin=" << getLibPinCapacitance(arc.cell->footprintIndex, bestType, arc.pin)
					<< " error= " << bestError
					<< " fanout=" << net.fanout
...
void Circuit::logicalEffort_Discretize() {
	const int numNets = timingNets.size();
	for (int n = numNets; n >= timingOffsetToNetLevel[1]; n--) { // skip dummy and nets driven by ffs.
...
void Circuit::logicalEffort_Test() {
	...
	for (int i = 0; i < 10; i++) {
		...
		std::cout << "worst arrival (timing): " <<
				logicalEffortWorstArrivalTime << "\n";
	} // end for
...
int Circuit::updateDelayScalingFactors_PathTrace(const int nstart) {
	...
	while (nstop >= timingNumDummyNets) {
		...
		if (!approximatelyEqual(slack.getMin(), slackThreshold))
			break;
...
void Circuit::analysis_Cells() {
	...
	for (int k = offsetCombinational; k < numCells; k++) {
		...
		file << setw(25) << cell->instName << " "
				<< setw(10) << cell->instType << " "
				<< "size=" << cellChar.size << " "
				<< "vth=" << cellChar.vth << " "
...
void Circuit::dumpVtStats(const std::string &filename) {
	...
	for (int k = offsetCombinational; k < numCells; k++) {
		...
		out
				...
				<< std::setw(6) << net.fanout << ", "
				<< std::setw(12) << net.centrality << ", "
				<< std::setw(6) << cell->logicalDepth << ", "
				<< std::setw(6) << cell->reverseLogicalDepth << ", "
				<< std::setw(12) << net.wipaths << ", "
				<< std::setw(12) << net.wopaths << "\n";
